...
![Status](https://img.shields.io/badge/Status-Pre--release%20(Under%20Review)-orange)  
...
The current release provides desert-wide dune instance products and derived geometric/dynamic attributes for scientific evaluation and project demonstration.

### 中文
本项目面向**巴丹吉林沙漠**复杂高大沙山（Megadunes）的高精度实例分割与三维形态参数提取。  
我们提出了一种新型端到端深度学习框架，**风沙动力学先验**（主风向、背风坡演化规律、休止角约束）与**物理约束可微分元胞自动机（Differentiable CA）**相结合。  

当前版本发布了全沙漠尺度的沙山实例分割成果及其几何—动力学属性参数，用于学术评估与结项展示。
...
> [!WARNING]  
> **English**  
> The original Physics-Informed Differentiable Cellular Automata network architecture is currently **under SCI journal review** (*Under Review / Pre-release*).  
...
> The complete codebase will be fully open-sourced after paper acceptance.
>
> **中文**  
> 本项目底层原创“物理约束可微分元胞自动机”网络架构目前处于**SCI 顶刊投稿审稿阶段**（*Under Review / Pre-release*）。  
> 为保护核心知识产权，**完整 PyTorch 训练源码**与**预训练权重**暂不公开。  
> 当前仓库 Releases 页面仅首发 **V1.0 沙丘实例分割与形态学属性数据**，供学术评估与结项展示使用。  
> 论文接收后将全面开源完整代码体系。
...
- (If provided in release assets) associated raster products in `.tif` format.
...
```bibtex
@dataset{badainjaran_megadune_v1,
  title        = {Badain Jaran Megadune Atlas: Instance Segmentation and Morphometric Database (V1.0 Pre-release)},
  author       = {Your Name and Collaborators},
  year         = {2026},
  publisher    = {GitHub},
  howpublished = {\url{https://github.com/fyichen2023/BadainJaran-Dune-Seg}},
  note         = {Data-only pre-release; full model code to be released after manuscript acceptance}
}
```
